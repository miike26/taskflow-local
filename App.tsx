...
const AppContent = () => {
    ...
    // 👇 INSERÇÃO 1A: GERENCIADOR GLOBAL DE TAGS
    const syncCustomTags = useCallback((taskTags: string[] | undefined) => {
        if (!taskTags || taskTags.length === 0) return;

        const currentDict = appSettings?.customTags || [];
        let dictionaryChanged = false;
        const newDict = [...currentDict];

        taskTags.forEach(tag => {
            const sanitized = sanitizeTag(tag);
            if (sanitized && !newDict.includes(sanitized)) {
                newDict.push(sanitized);
                dictionaryChanged = true;
            }
        });

        if (dictionaryChanged) {
            updateUserDoc({ appSettings: { ...appSettings, customTags: newDict } });
        }
    }, [appSettings, updateUserDoc]);

    // 👇 INSERÇÃO 1B: A MIGRAÇÃO FANTASMA
    useEffect(() => {
        // Se já carregou userData E a propriedade customTags nunca foi criada
        if (userData && tasks.length > 0 && appSettings.customTags === undefined) {
            console.log("Iniciando migração fantasma de tags...");

            const allExtractedTags = tasks.flatMap(t => t.tags || []);

            const uniqueSanitizedTags = Array.from(
                new Set(
                    allExtractedTags
            ...
            updateUserDoc({ appSettings: { ...appSettings, customTags: uniqueSanitizedTags } });
            console.log(`Migração concluída: ${uniqueSanitizedTags.length} tags encontradas.`);
        }
    }, [userData, tasks, appSettings, updateUserDoc]);
    ...
    const handleDeleteReminderRequest = (taskId: string, reminderId: string) => {
        setConfirmationState({
            isOpen: true, title: 'Excluir Lembrete', message: 'Tem certeza que deseja excluir este lembrete?', onConfirm: () => {
                const task = tasks.find(t => t.id === taskId);
