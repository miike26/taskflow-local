...
}> = ({ title, description, icon, reversed, mediaPlaceholderText }) => (
    <div className={`flex flex-col lg:flex-row items-center gap-16 py-24 ${reversed ? 'lg:flex-row-reverse' : ''}`}>
        ...
        <div className="flex-1 w-full relative group perspective-1000">
            ...
            <div className="relative w-full aspect-video bg-white/50 dark:bg-[#161B22]/80 backdrop-blur-md rounded-2xl shadow-2xl border border-white/50 dark:border-white/10 overflow-hidden transform transition-all duration-500 group-hover:scale-[1.02] group-hover:shadow-primary-500/10">
                {/* Internal UI Mockup */}
                <div className="absolute top-0 left-0 w-full h-10 bg-gray-100/50 dark:bg-white/5 border-b border-gray-200 dark:border-white/5 flex items-center px-4 gap-2">
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-[#090c10] text-gray-800 dark:text-gray-200 font-sans overflow-x-hidden selection:bg-primary-500 selection:text-white relative">
            ...
            <header className="relative pt-48 pb-32 px-6 overflow-hidden">
                ...
                <div className="mt-24 max-w-7xl mx-auto relative animate-scale-in perspective-[2000px] group">
                    ...
                    <div className="relative aspect-[16/10] bg-gray-900 rounded-2xl shadow-2xl border border-white/10 overflow-hidden transform rotate-x-12 group-hover:rotate-x-0 transition-transform duration-1000 ease-out origin-bottom">
                         {/* Mockup Header */}
                         ...
                         <div className="absolute inset-0 pt-12 bg-[#0D1117] flex items-center justify-center">
                            <div className="text-center p-8 border-2 border-dashed border-gray-700/50 rounded-2xl">
                                <span className="inline-block px-6 py-3 rounded-lg bg-white/5 text-gray-500 font-bold uppercase tracking-widest text-sm border border-white/5">
                                    Imagem do Dashboard
                                </span>
            ...
            <section className="py-24 bg-white dark:bg-[#0D1117] border-t border-gray-200 dark:border-gray-800 relative z-10">
                <div className="max-w-4xl mx-auto px-6">
                    ...
                    <div className="bg-gray-50 dark:bg-[#161B22]/50 p-10 rounded-3xl border border-gray-200 dark:border-gray-800 h-96 overflow-y-auto custom-scrollbar prose prose-lg dark:prose-invert max-w-none shadow-inner">
                        {activeTab === 'terms' ? (
                            <div>
                                <h3>1. Aceitação dos Termos</h3>
                                <p>Ao acessar e usar o FlowTask, você concorda em cumprir estes Termos de Uso e todas as leis aplicáveis. Se você não concordar com algum destes termos, está proibido de usar ou acessar este site.</p>
                                
                                <h3>2. Licença de Uso</h3>
                                <p>Concedemos a você uma licença limitada, não exclusiva e intransferível para usar o aplicativo para fins pessoais ou profissionais de organização. Esta licença não permite a revenda do software.</p>

                                <h3>3. Contas de Usuário</h3>
                                <p>Você é responsável por manter a confidencialidade de sua conta e senha. O FlowTask não se responsabiliza por perdas decorrentes do uso não autorizado de sua conta.</p>

                                <h3>4. Uso da IA</h3>
                                <p>Os recursos de Inteligência Artificial utilizam a API do Google Gemini. Ao ativar esses recursos, você concorda que os dados enviados (textos de tarefas) sejam processados para gerar as respostas. Nenhuma garantia de precisão é dada sobre o conteúdo gerado por IA.</p>
                            </div>
                        ) : (
                            <div>
                                <h3>1. Coleta de Dados</h3>
                                <p>Coletamos apenas as informações estritamente necessárias para o funcionamento do app: nome, email (opcional para login social) e os dados de tarefas que você insere voluntariamente.</p>

                                <h3>2. Armazenamento</h3>
                                <p>Seus dados são armazenados primariamente de forma local no seu dispositivo (LocalStorage) para persistência básica e, quando conectado a uma conta, em nossos servidores seguros criptografados.</p>

                                <h3>3. Dados e IA</h3>
                                <p>Os dados enviados para a IA (Google Gemini) são usados estritamente para gerar a resposta solicitada (resumo, sugestão) no momento da requisição e não são armazenados pelos modelos de IA para treinamento público, conforme política de privacidade da API do Google Cloud.</p>

                                <h3>4. Seus Direitos</h3>
                                <p>Você detém total propriedade sobre seus dados. Pode solicitar a exportação ou exclusão completa de sua conta e todos os dados associados a qualquer momento através do painel de configurações.</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <footer className="py-12 text-center border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-[#0D1117] relative z-10">
                <div className="flex items-center justify-center gap-2 mb-4 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
                    <img src={APP_ICON_URL} alt="FlowTask" className="h-6 w-auto" />
